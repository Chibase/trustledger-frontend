...
1. Read `.ai/TASK.md` and `.ai/HANDOFF.md` before implementation. TASK lifecycle: **EMPTY → ASSIGNED → IN PROGRESS → COMPLETE → VERIFIED → CLOSED** (defined in `.ai/README.md`). **EMPTY, COMPLETE, VERIFIED, CLOSED = do not implement.** Execute only when Status is **ASSIGNED** (then set IN PROGRESS) or **IN PROGRESS**, and only the TASK body. Do not infer a packet from BUILD_PLAN. Do not set VERIFIED or CLOSED. Then read `docs/BUILD_PLAN.md`. Workflow: `.ai/README.md`.
...
7. Product name in UI: **TrustLedger** only. Public voice = **Trust** (ADR-039). No Frappe/Vercel/HubSpot (or other stack brands) in marketing, FAQ, or public agents — say TrustLedger Cloud / cloud. Chibase = footer/legal/ops only.
8. Live data mode may fall back to mock if Frappe is unreachable — **never** for customer/trial workspaces (no demo INC-* bleed). Platform **ZA place packs** are shared baseline intel (ADR-040), not fictional case seed — see `docs/ZA_BASELINE_INTEL.md`.
