...
describe("TE-6 MVP packaging", () => {
  ...
  it("keeps future-only and do-not-promise lists explicit", () => {
    ...
    expect(TRUST_MVP_COMPLETE.join(" ")).toMatch(/TE-12/i);
  ...
  it("does not claim proof lives only on reports or that packaging is product-complete", () => {
    ...
    expect(partial).toMatch(/linked evidence plus human apply/i);
    expect(partial).not.toMatch(/no separate trust-claim verification/i);
    expect(partial).toMatch(/not statistical causality/i);
    ...
    expect(complete).toMatch(/TE-12/i);
  });
});
