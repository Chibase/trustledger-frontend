...
| **SI-SEP** | Stakeholder engagement plan | RFP/tender/briefing **or facts pack (no file)** → extract metadata → seven-phase dashboard + Gemini-drafted tender SEP (template fallback; PDF/MD/Word); human apply seeds registry / engagements / commitments. Analysis engine (Phases A–G): data model, tender parser, social context, method library, compliance matrix, participation planner, 14 QA tests, 25-section renderer | **Active** |
