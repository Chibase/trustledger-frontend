...
- Commercial plan packages **module dashboards** in a fixed tier sequence (`TIER_FLOW`). SEP is a module, not the plan. Executive landing shows per-module contribution and aggregate progress. Persistent **Plan modules** switcher (executive ↔ module in two clicks).
- VIP illustrative showcase seeds all included desks (including SEP-NCGR-B and ESG-NCGR-B) on login / session sync. Non-VIP and live Cloud VIP stay unseeded. `GET /api/app/plans/dashboards`. Legacy `/plans/:id/sep` → `/app/engagement-plan`. ADR-056.
