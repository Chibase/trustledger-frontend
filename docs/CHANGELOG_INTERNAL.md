...
- Trial SI lists (engagements, stakeholders, commitments) keep local/org rows when live Cloud SI returns empty. Live Cloud customers still do not fill an empty Cloud with leftover trial/showcase rows.
...
- What worked uses on-track expected vs actual (project and commitment), closed cases, and done Learn & Adapt records. What did not uses MEL-1 shortfall watches, open tagged cases, and open records. What we will change lists open Adapt actions and does not invent a change when none are on file.
