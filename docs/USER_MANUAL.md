...
On the **Executive dashboard** (`/app/dashboard`), the **Plan modules** strip is the sequence for your commercial plan (executive first, then the desks that plan includes). Open a module, then **Return to executive view**. Module cards on the executive page show contribution to overall progress. **Engagement plan** is one module on Project+ — not the whole product. VIP showcase workspaces include illustrative NCGR-B rows on those desks; other plans start empty and you populate them.
