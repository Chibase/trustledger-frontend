...
- **Decision:**
  1. Keep **Engagement plan** under `/app/engagement-plan` as an **operator / VIP desk** while the composer is built — not gated on `engagements`, and not a commercial SKU. Customer plans (Solo / Practitioner / Project / Institutional / demo Project lens) do not see nav, dashboards, onboarding, or APIs. `/ops` stays the operator home. No new entitlement; Institutional Settings must not force it on.
