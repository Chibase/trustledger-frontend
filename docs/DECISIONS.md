...
- **Decision:**
  ...
  3. Outputs: an operator **dashboard** (power–interest; Social Licence to Build™ → shipped desks — internal only) and a client-presentable **report** (executive summary; regulatory mapping; stakeholder identification process; operational methodology including CBPR; risk and GRM; M&E; assumptions; conclusion). The report states what will be done, how, when, and by whom. Letterhead is TrustLedger + Chibase Consulting. **Do not** describe product architecture, desks, Themba, Capture, Apply, or “execution protocols” in anything a client will read. Capabilities are demonstrated by the plan of work, not explained. Branded PDF, Markdown, Word-compatible `.doc`, and print layout.
