...
- **Deepened 2026-08-25 (SEC-1):** Plan Owner Cloud Users are stamped with a Customer User Permission (`apply_to_all_doctypes`). Live SI / migrate / upload BFF bind Customer from the sign-in, not a client-supplied name (Platform Operators may break-glass). Invitee Cloud seats remain **SEC-5**. Sales copy may say server-side organisation enforcement for Plan Owners; do not claim every junior seat is Cloud-permissioned.
