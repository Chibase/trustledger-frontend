...
- **Deepened 2026-08-25 (SEC-1):** Plan Owner Cloud Users are stamped with a Customer User Permission (`apply_to_all_doctypes`). Live SI / migrate / upload / project BFF bind Customer from the **Cloud sid**, not a client-supplied name or a forgeable email cookie (Platform Operators may break-glass). Buyer `customer=` is ignored so first-login migrate still works when the browser org label differs from the Cloud Customer name.
- **Deepened 2026-09-05 (SEC-5):** Accepted teammates on a live organisation get their own Cloud User on that Customer (same User Permission; `custom_tl_plan_owner=0`; locked desk; `custom_tl_app_role`). Live bind uses Owner email first, then invitee `User.custom_tl_customer`. Trial/browser-only invites without a Cloud Customer stay device-local. Sales copy may say accepted teammates on a live organisation get their own Cloud login bound to that organisation; do not claim SOC 2, dedicated isolation, or that trial seats are Cloud Users.
...
- **Decision:**
  ...
  4. After approval, **Apply to SRM** creates prospect stakeholders, draft engagements, and open commitments via existing services. Duplicate names/titles are skipped. Live entitled workspaces persist the plan (and the plan-centric execution overlay) on **TL Engagement Plan**; trial without a Cloud Customer stays org-scoped in the browser. Applied SI rows follow the live SI path. Empty Cloud stays empty.
...
- **Deepened 2026-09-05:** Cloud SoT is `TL Engagement Plan` (`GET|POST|DELETE /api/frappe/sep`). Indexed fields + JSON `payload` / `execution_json`. Local `tl-engagement-plans` is a cache on live. Gemini still drafts the presentable document only (suggest → apply → save). Not a sealed ledger.
...
- **Decision:**
  ...
  2. Overlay lives on **TL Engagement Plan**.`execution_json` for live entitled workspaces. Trial / VIP seed stay in the browser (`tl-sep-execution`) until go-live migrate.
...
- **Consequences:** No new Paystack SKU. No SQL. Live persist uses the same DocType as the composer (SI-SEP Cloud persist).
- **Deepened 2026-09-05:** `GET|POST /api/frappe/sep` writes overlay only when the client sends it (PUT omits `execution_json` otherwise). Empty Cloud stays empty.
...
- **Decision:**
  1. `TIER_FLOW` defines allowed modules, sequence, and advisory gates per `PlanId`. VIP complimentary uses the Institutional sequence.
  2. Executive dashboard (`/app/dashboard`) is the plan landing; the module switcher is always visible. Module ↔ executive is two clicks. Entitled modules stay reachable (gates never hide a desk so non-VIP can populate immediately). Unauthorized module URLs show the plan upsell.
  3. Demo seed (`applyVipShowcaseSeed`) runs only for Thozamile KaDlanga’s trial+VIP showcase mailbox (`thozi@chibaseconsulting.co.za`). Live Cloud VIP, other complimentary VIP guests, and paying/trial workspaces are never seeded with NCGR-B; leftover theatre is purged.
