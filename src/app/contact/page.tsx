...
import { PRIVACY_EXTRAS, type PrivacyExtraId } from "@/config/planComparison";

const EXTRA_IDS = new Set(PRIVACY_EXTRAS.map((e) => e.id));
...
function ContactForm() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      const plan = planRaw && isPlanId(planRaw) ? planRaw : null;
      const extras = (extrasRaw ?? "")
        .split(",")
        .map((s) => s.trim())
        .filter((id): id is PrivacyExtraId => EXTRA_IDS.has(id as PrivacyExtraId));

      if (!plan && extras.length === 0) return;

      const lines: string[] = [];
      if (plan) {
        lines.push(`I am interested in the ${PLANS[plan].name} plan.`);
      }
      if (extras.length) {
        const names = extras
          .map((id) => PRIVACY_EXTRAS.find((e) => e.id === id)?.name ?? id)
          .join(", ");
        lines.push(`Optional privacy layers: ${names}.`);
      }
      lines.push("Please follow up with options and next steps.");
      setMessage(lines.join("\n"));
    }, 0);
    return () => window.clearTimeout(timer);
    // Prefill once from the landing URL; do not overwrite user edits.
    // eslint-disable-next-line react-hooks/exhaustive-deps -- intentional one-shot from searchParams
  }, [searchParams]);
...
export default function ContactPage() {
  return (
    <main className="mx-auto flex min-h-full max-w-lg flex-col justify-center px-4 py-12">
      ...
      <Suspense
        fallback={
          <p className="mt-8 text-sm text-tl-ink-muted">Loading contact form…</p>
        }
      >
        <ContactForm />
      </Suspense>
